...
def run_mixffn_tests():
    ...
    for in_f, hidden_f, out_f, b, s, h, w, block_i, mixffn_i in test_configs:
        ...
        try:
            ...
            # Mock params in TT-Metal preprocessed orientation: [in, out]
            mock_params = {
                "dense1": {
            ...
            if list(output.shape) == expected_shape:
                print(f"[PASSED] {test_name} -> Output Shape: {list(output.shape)}")
            else:
                print(f"[FAILED] {test_name} -> Expected {expected_shape}, got {list(output.shape)}")
                all_passed = False
