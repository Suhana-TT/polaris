...
from workloads.EA_LSS.ttsim_modules.clip_sigmoid             import clip_sigmoid          # Sigmoid clamped to (0,1) used by heatmap head
from workloads.EA_LSS.ttsim_modules.gaussian                 import gaussian_radius, draw_heatmap_gaussian    # Gaussian heatmap target generation
from workloads.EA_LSS.ttsim_modules.coord_transform          import apply_3d_transformation                # Camera↔LiDAR coordinate transforms
from workloads.EA_LSS.ttsim_modules.box3d_nms                import box3d_multiclass_nms                   # 3-D NMS for detection output
from workloads.EA_LSS.ttsim_modules.transfusion_bbox_coder   import TransFusionBBoxCoder                   # Box encode/decode for TransFusion

# Level 2 – Sub-network building blocks
from workloads.EA_LSS.ttsim_modules.norm                     import NaiveSyncBatchNorm1d, NaiveSyncBatchNorm2d, NaiveSyncBatchNorm3d  # Sync BatchNorm wrappers
from workloads.EA_LSS.ttsim_modules.mlp                      import MLP as _MLP          # Generic multi-layer perceptron
from workloads.EA_LSS.ttsim_modules.ffn                      import FFN                   # Feed-Forward Network (Transformer FFN)
from workloads.EA_LSS.ttsim_modules.multihead_attention      import MultiheadAttention as _MHA  # Multi-head self/cross attention
from workloads.EA_LSS.ttsim_modules.position_embedding_learned import PositionEmbeddingLearned  # Learned 2-D positional embedding
from workloads.EA_LSS.ttsim_modules.se_block                 import SE_Block              # Squeeze-and-Excitation channel attention

# Level 3 – Composite sub-modules
from workloads.EA_LSS.ttsim_modules.swin_transformer         import SwinTransformer       # Standard Swin Transformer backbone
from workloads.EA_LSS.ttsim_modules.voxel_encoder_utils      import VFELayer, get_paddings_indicator  # VFE building blocks
from workloads.EA_LSS.ttsim_modules.transfusion_head         import TransformerDecoderLayer, TransFusionHead  # Detection head

# Level 4 – Full functional modules
from workloads.EA_LSS.ttsim_modules.fpn                      import FPN                   # Feature Pyramid Network (used by FPNC)
from workloads.EA_LSS.ttsim_modules.fpnc                     import FPNC, gapcontext      # Extended FPN for camera stream (gap context + multi-scale concat)
from workloads.EA_LSS.ttsim_modules.cbnet                    import CBSwinTransformer     # Composite Backbone: dual-CBSwin w/ cross-backbone fusion
from workloads.EA_LSS.ttsim_modules.voxel_encoder            import HardSimpleVFE, HardSimpleVFE_ATT   # LiDAR voxel feature encoders
from workloads.EA_LSS.ttsim_modules.second                   import SECOND                # Stacked Encoder Conv blocks for sparse BEV
from workloads.EA_LSS.ttsim_modules.second_fpn               import SECONDFPN             # FPN neck on top of SECOND, merges 3 strides
from workloads.EA_LSS.ttsim_modules.cam_stream_lss           import LiftSplatShoot        # Camera→BEV via lift-splat-shoot
from workloads.EA_LSS.ttsim_modules.cam_stream_lss_quickcumsum import QuickCumsum         # Fast cumulative sum kernel (used by LSS)

# Level 5–6 – Base detector classes (kept for inheritance context)
from workloads.EA_LSS.ttsim_modules.base_3d_detector         import Base3DDetector        # Abstract 3-D detector shell
from workloads.EA_LSS.ttsim_modules.mvx_two_stage            import MVXTwoStageDetector   # Generic multi-modal orchestrator
from workloads.EA_LSS.ttsim_modules.mvx_faster_rcnn          import MVXFasterRCNN, DynamicMVXFasterRCNN  # FasterRCNN-style detector wrappers
from workloads.EA_LSS.ttsim_modules.transfusion_detector     import TransFusionDetector   # TransFusion top-level (alternative to EALSS)
...
class EALSS(SimNN.Module):
    ...
    def __init__(
        ...
        camera_depth_range: list | None = None,
        pc_range: list | None = None,
