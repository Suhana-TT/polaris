...
# Optional imports for training (not needed for validation tests)
try:
    ...
    if not TYPE_CHECKING:
        # Provide minimal stubs for validation tests
        ...
        class TrajectorySampling:  # type: ignore[no-redef]
            def __init__(
                self, time_horizon: float = 4.0, interval_length: float = 0.5, **kwargs
            ):
...
class TransfuserConfig:
    ...
    bkb_path: str = os.path.expanduser(
        os.getenv(
            "DIFFUSIONDRIVE_RESNET34_CKPT",
            "~/.cache/huggingface/hub/checkpoints/resnet34.a1_in1k/pytorch_model.bin",
        )
    )
    plan_anchor_path: str = field(
        default_factory=lambda: os.path.normpath(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "..",
                "..",
                "kmeans_navsim_traj_20.json",
            )
        )
    )
    plan_anchor_fallback_seed: int = 42
