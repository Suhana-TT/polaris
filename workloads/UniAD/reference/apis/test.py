...
def collect_results_gpu(result_part, size, tmpdir=None):
    """GPU-based result collection wrapper.
    Currently delegates to ``collect_results_cpu`` to avoid breaking callers
    expecting a non-None return value.
    """
    return collect_results_cpu(result_part, size, tmpdir)
