...
class TrajLoss(nn.Module):
    ...
    def forward(self, traj_prob, traj_preds, gt_future_traj, gt_future_traj_valid_mask):
        ...
        try:
            ...
            # Fall back to a zero miss rate if miss_rate computation fails
            l_mr = torch.tensor(0.0, dtype=l_minfde.dtype, device=l_minfde.device)
...
def traj_nll(pred_dist: torch.Tensor, traj_gt: torch.Tensor, masks: torch.Tensor):
    ...
    # Ensure positive standard deviations and valid correlation
    raw_sig_x = pred_dist[:, :, 2]
    raw_sig_y = pred_dist[:, :, 3]
    raw_rho = pred_dist[:, :, 4]
    eps = 1e-6
    sig_x = F.softplus(raw_sig_x) + eps
    sig_y = F.softplus(raw_sig_y) + eps
    rho = torch.tanh(raw_rho)
    one_minus_rho2 = 1.0 - torch.pow(rho, 2)
    ohr = torch.pow(one_minus_rho2, -0.5)
    nll = (
        0.5
        * torch.pow(ohr, 2)
        * (
            torch.pow(x - mu_x, 2) / torch.pow(sig_x, 2)
            + torch.pow(y - mu_y, 2) / torch.pow(sig_y, 2)
            - 2
            * rho
            * (x - mu_x)
            * (y - mu_y)
...
def miss_rate(
    ...
    # Handle empty batch safely: return a well-defined scalar miss rate.
    if dist.numel() == 0:
        return torch.tensor(0.0, device=traj.device, dtype=traj.dtype)
    # Compute miss rate as the mean over the batch of the "missed" condition.
    m_r = (dist > dist_thresh).to(traj.dtype).mean()
