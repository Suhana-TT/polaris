...
class MotionNonlinearSmoother:
    ...
    def set_reference_trajectory(
        self, x_curr: Sequence[float], reference_trajectory: Sequence[Pose]
    ) -> None:
        """
    ...
    def _set_state_constraints(self) -> None:
        ...
        max_speed = 35.0  # m/s
        self._optimizer.subject_to(
            self._optimizer.bounded(0.0, self.speed, max_speed)
        )  # only forward
        max_yaw_rate = 1.75  # rad/s
        self._optimizer.subject_to(
            self._optimizer.bounded(
                -max_yaw_rate, diff(self.yaw) / self._dts, max_yaw_rate
            )
        )
        max_lateral_accel = (
            4.0  # m/s^2, assumes circular motion acc_lat = speed^2 * curvature
        )
        self._optimizer.subject_to(
            self._optimizer.bounded(
                -max_lateral_accel,
    ...
    def _set_objective(self) -> None:
        ...
        cost_stage = (
            alpha_xy
            * sumsqr(self.ref_traj[:2, :] - vertcat(self.position_x, self.position_y))
        ...
        alpha_terminal_yaw = (
            40.0  # really care about final heading to help with lane changes
        )
    ...
    def _set_initial_guess(
        ...
        self._optimizer.set_initial(
            self.state[:3, :], DM(reference_trajectory).T
        )  # (x, y, yaw)
