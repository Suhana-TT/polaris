...
from workloads.ttnn.vadv2.tt import tt_vad
from workloads.ttnn.vadv2.reference.base_box3d import LiDARInstance3DBoxes
from workloads.ttnn.vadv2.tests.pcc.vad_param import create_vadv2_model_parameters_vad
...
def test_vadv2():
    ...
    input_dict = {
        ...
        "gt_bboxes_3d": [
            [
                [
                    LiDARInstance3DBoxes( # type: ignore[abstract]
                        ttnn.Tensor(shape=[11, 9], device=device, data=np.array([
        ...
        "gt_labels_3d": [[[ttnn.Tensor(shape=[11,], device=device, data=np.array([8, 8, 8, 8, 8, 8, 0, 8, 8, 0, 8]))]]],
        "fut_valid_flag": [ttnn.Tensor(shape=[1,], device=device, data=np.array([True]))],
        "ego_his_trajs": [[ttnn.Tensor(shape=[1, 1, 2, 2], device=device, data=np.array([[[[0.0757, 4.2529], [0.0757, 4.2529]]]]))]],
        "ego_fut_trajs": [[ttnn.Tensor(shape=[1, 1, 6, 2], device=device, data=np.zeros((1, 1, 6, 2)), dtype=ttnn.float32)]],
        "ego_fut_masks": [[ttnn.Tensor(shape=[1, 1, 6], device=device, data=np.ones((1, 1, 6)), dtype=ttnn.float32)]],
        "ego_fut_cmd": [[ttnn.Tensor(shape=[1, 1, 1, 3], device=device, data=np.array([[[[1.0, 0.0, 0.0]]]]), dtype=ttnn.float32)]],
        "ego_lcf_feat": [[ttnn.Tensor(shape=[1, 1, 1, 9], device=device, data=np.zeros((1, 1, 1, 9)), dtype=ttnn.float32)]],
        "gt_attr_labels": [
            [
                [
                    ttnn.Tensor(shape=[11, 34], device=device, data=np.array([
                            [
                                -1.6727e-01,
                                -7.2574e-01,
                                -1.6727e-01,
        ...
        "map_gt_labels_3d": [[ttnn.Tensor(shape=[7,], device=device, data=np.zeros((7,)), dtype=ttnn.int64)]],
        "map_gt_bboxes_3d": [[None]],
    }
    tensor = ttnn._rand(shape= [1, 6, 3, 384, 640], dtype=ttnn.float32, device=device)
    img = []
    img.append(tensor)
    parameter = create_vadv2_model_parameters_vad(device)
    ...
    logger.debug('VADv2 input shape is ', img[0].shape)
    if (img[0].shape == [1, 6, 3, 384, 640] and
        ttnn_outputs[0]['pts_bbox']['scores_3d'].shape == [100] and
        ttnn_outputs[0]['pts_bbox']['labels_3d'].shape == [100] and
        ttnn_outputs[0]['pts_bbox']['trajs_3d'].shape == [100, 6] and
        ttnn_outputs[0]['pts_bbox']['map_boxes_3d'].shape == [50, 4] and
        ttnn_outputs[0]['pts_bbox']['map_scores_3d'].shape == [50] and
