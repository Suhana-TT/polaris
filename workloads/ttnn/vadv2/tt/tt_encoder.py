...
from workloads.ttnn.vadv2.tt.tt_temporal_self_attention import TtTemporalSelfAttention
from workloads.ttnn.vadv2.tt.tt_spatial_cross_attention import TtSpatialCrossAttention
from workloads.ttnn.vadv2.tt.tt_ffn import TtFFN
from workloads.ttnn.vadv2.tt.tt_utils import DictAsAttr


class TtBEVFormerEncoder(SimNN.Module):
    def __init__(
        self,
        params,
        device,
        ...
        self.name = "tt_bev_former_encoder"
        self.device = device
        self.params = params
    ...
    def point_sampling_ttnn(self, reference_points, pc_range, img_metas):
        ...
        x_add_val = ttnn.full(x.shape, pc_range[0], dtype=ttnn.bfloat16, device=self.device, layout=ttnn.Layout.TILE_LAYOUT)
        y_add_val = ttnn.full(y.shape, pc_range[1], dtype=ttnn.bfloat16, device=self.device, layout=ttnn.Layout.TILE_LAYOUT)
        z_add_val = ttnn.full(z.shape, pc_range[2], dtype=ttnn.bfloat16, device=self.device, layout=ttnn.Layout.TILE_LAYOUT)
        x = x * x_mul_val + x_add_val
    ...
    def __call__(
        ...
        **kwargs,
        ...
        shift = ttnn.Tensor(shape=shift.shape, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.Layout.TILE_LAYOUT, data=shift.data)
        ...
        for operation_name in self.operation_order:
            if operation_name in ["self_attn", "cross_attn"]:
                ...
                if attn_cfgs[index]["type"] == "TemporalSelfAttention":
                    ...
                    _ = attn_cfgs[index].pop("type")
                    attention = TtSpatialCrossAttention(device, params.attentions[f"attn1"], **attn_cfgs[index]) # type: ignore[assignment]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for layer in self.operation_order:
            if layer == "self_attn":
                spatial_shapes_1 = ttnn.Tensor(shape=(1,2), dtype=ttnn.uint32, device=self.device,
                                               layout=ttnn.ROW_MAJOR_LAYOUT, data=np.array([bev_h, bev_w]))
                ...
                if isinstance(params.norms[f"norm{norm_index}"], dict): # type: ignore[attr-defined]
                    params_norms = DictAsAttr(params.norms[f"norm{norm_index}"]) # type: ignore[attr-defined]
