...
from workloads.ttnn.vadv2.reference.base_box3d import LiDARInstance3DBoxes
from workloads.ttnn.vadv2.tt.tt_utils import inverse_sigmoid, bbox_xyxy_to_cxcywh
from workloads.ttnn.vadv2.reference.nms_free_coder import MapNMSFreeCoder, CustomNMSFreeCoder
...
class TtLearnedPositionalEncoding:
    ...
    def __call__(self, mask):
        ...
        params_col_embed = self.params['col_embed']
        x_embed = self.col_embed(
            x,
            params_col_embed["weight"],
            layout=ttnn.TILE_LAYOUT,
        )
        params_row_embed = self.params['row_embed']
        y_embed = self.row_embed(y, params_row_embed["weight"], layout=ttnn.TILE_LAYOUT)
        ...
        out = ttnn.concat(x_embed, y_embed, axis=-1, memory_config=ttnn.L1_MEMORY_CONFIG)
...
class TtVADHead(SimNN.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # print(f'Positional Encoding params: {self.params}')
        params = DictAsAttr(params)
        params_head_positional_encoding = params.head['positional_encoding']

        self.positional_encoding = TtLearnedPositionalEncoding(
            params_head_positional_encoding, device, self.embed_dims // 2, row_num_embed=100, col_num_embed=100
        )

        params_head_transformer = params.head['transformer']
        params_head_branches = params.head['branches']
        self.transformer = TtVADPerceptionTransformer(
            params=params_head_transformer,
            params_branches=params_head_branches,
            device=device,
        ...
        # print(f'Transformer initialized: {self.transformer}')
        ...
        params = DictAsAttr(self.params)
        if not self.as_two_stage:
            params_head_bev_embedding = params.head['bev_embedding']
            self.bev_embedding = params_head_bev_embedding
            params_head_query_embedding = params.head['query_embedding']
            self.query_embedding = params_head_query_embedding

            if self.map_query_embed_type == "all_pts":
                params_head_map_query_embedding = params.head['map_query_embedding']
                self.map_query_embedding = params_head_map_query_embedding
            elif self.map_query_embed_type == "instance_pts":
                self.map_query_embedding = None
                params_head_map_instance_embedding = params.head['map_instance_embedding']
                self.map_instance_embedding = params_head_map_instance_embedding
                params_head_map_pts_embedding = params.head['map_pts_embedding']
                self.map_pts_embedding = params_head_map_pts_embedding

        if self.motion_decoder is not None:
            params_head_motion_decoder = params.head['motion_decoder']
            self.motion_decoder = TtCustomTransformerDecoder(params_head_motion_decoder, self.device, num_layers=1)
            self.motion_mode_query = params.head['motion_mode_query']
            if self.use_pe:
        ...
        if self.motion_map_decoder is not None:
            params_head_lane_encoder = params.head['lane_encoder']
            self.lane_encoder = TtLaneNet(params_head_lane_encoder, self.device, 256, 128, 3)

            self.motion_map_decoder = TtCustomTransformerDecoder(
                params.head['motion_map_decoder'], self.device, num_layers=1
            )
            if self.use_pe:
        ...
        if self.ego_agent_decoder is not None:
            params_head_ego_agent_decoder = params.head['ego_agent_decoder']
            self.ego_agent_decoder = TtCustomTransformerDecoder(
                params_head_ego_agent_decoder, self.device, num_layers=1
            )
            if self.use_pe:
                self.ego_agent_pos_mlp = ttnn.linear

        if self.ego_map_decoder is not None:
            params_head_ego_map_decoder = params.head['ego_map_decoder']
            self.ego_map_decoder = TtCustomTransformerDecoder(
                params_head_ego_map_decoder, self.device, num_layers=1
            )
            if self.use_pe:
    ...
    def __call__(
        ...
        bev_mask = ttnn.zeros((bs, self.bev_h, self.bev_w), device=self.device, dtype=ttnn.bfloat16, layout=ttnn.ROW_MAJOR_LAYOUT)
        ...
        for lvl in range(hs.shape[0]):
            reference = init_reference if lvl == 0 else ttnn.Tensor(shape=(inter_references.shape[1], inter_references.shape[2], inter_references.shape[3]), dtype=ttnn.bfloat16, device=self.device)
            reference = inverse_sigmoid(reference)

            cls_layers = self.params["head"]["branches"]["cls_branches"][str(lvl)]
            cls_tmp = ttnn.Tensor(shape=(hs_shape[1], hs_shape[2], hs_shape[3]), dtype=ttnn.bfloat16, device=self.device)
            ...
            # tmp = hs[lvl]
            tmp = ttnn.Tensor(shape=(hs_shape[1], hs_shape[2], hs_shape[3]), dtype=ttnn.bfloat16, device=self.device)
            ...
            updated_xy = t2 + r2 # type: ignore[operator]
            updated_xy = ttnn.sigmoid(updated_xy)
            t45 = ttnn.Tensor(shape=(tmp_shape[0], tmp_shape[1], 1), dtype=ttnn.bfloat16, device=self.device)
            r23 = ttnn.Tensor(shape=(ref_shape[0], ref_shape[1], 1), dtype=ttnn.bfloat16, device=self.device)
            updated_z = t45 + r23 # type: ignore[operator]
            updated_z = ttnn.sigmoid(updated_z)

            outputs_coords_bev.append(ttnn.clone(updated_xy, memory_config=ttnn.L1_MEMORY_CONFIG))

            uxy = ttnn.Tensor(shape=(updated_xy.shape[0], updated_xy.shape[1], 1), dtype=ttnn.bfloat16, device=self.device)
            x = uxy * (self.pc_range[3] - self.pc_range[0]) + self.pc_range[0]
            y = uxy * (self.pc_range[4] - self.pc_range[1]) + self.pc_range[1]
            z = updated_z * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2]

            t5 = ttnn.Tensor(shape=(tmp_shape[0], tmp_shape[1], (tmp_shape[2] - 5)), dtype=ttnn.bfloat16, device=self.device)
            tmp_out = ttnn.concat(x, y, t2, z, t5, axis=-1)
        ...
        for lvl in range(map_hs.shape[0]):
            ...
            # === Class Branch ===
            map_hs_shape = map_hs.shape
            cls_input = ttnn.Tensor(shape=(map_hs_shape[1], map_hs_shape[2], map_hs_shape[3]), dtype=ttnn.bfloat16, device=self.device)
            ...
            reg_tmp = ttnn.Tensor(shape=(map_hs_shape[1], map_hs_shape[2], map_hs_shape[3]), dtype=ttnn.bfloat16, device=self.device)
            ...
            tmp_coord = t2 + r2 # type: ignore[operator]
            tmp_coord = ttnn.sigmoid(tmp_coord)

            tmp_full = ttnn.concat(tmp_coord, axis=-1)
        ...
        if self.motion_decoder is not None:
            ...
            # motion_query
            hs_m1 = ttnn.Tensor(shape=(hs.shape[1], hs.shape[2], hs.shape[3]), dtype=ttnn.bfloat16, device=self.device)
            motion_query = ttnn.permute(hs_m1, (1, 0, 2))  # [A, B, D]
            ...
            if self.motion_det_score is not None:
                ...
                max_motion_score = ttnn.max(motion_score, dim=-1)[0]
            ...
            if self.motion_map_decoder is not None:
                ...
                map_hs_m1 = ttnn.Tensor(shape=(map_hs.shape[1], map_hs.shape[2], map_hs.shape[3]), dtype=ttnn.bfloat16, device=self.device)
                map_query = ttnn.reshape(map_hs_m1, (batch_size, self.map_num_vec, self.map_num_pts_per_vec, -1))
                ...
                if self.use_pe:
                    ...
                    motion_pos = ttnn.zeros((num_query, batch, 2), device=self.device, layout=ttnn.TILE_LAYOUT, dtype=ttnn.bfloat16)
            ...
            motion_hs = ttnn.concat(motion_hs, ca_motion_query, axis=-1)  # [B, A, fut_mode, 2D]  #0.99
        else:
            raise NotImplementedError("Not implement yet")
        ...
        cls_tmp = motion_hs
        traj_cls_params = self.params['head']['branches']['traj_cls_branches']["0"]
        for i in range(0, 5, 2):
            cls_tmp = ttnn.linear(
                cls_tmp,
        ...
        ego_pos = ttnn.zeros((batch, 1, 2), device=self.device, layout=ttnn.TILE_LAYOUT, dtype=ttnn.bfloat16)
        ego_pos_emb = ttnn.linear(
            ego_pos, self.params['head']['ego_agent_pos_mlp']['weight'], bias=self.params['head']['ego_agent_pos_mlp']['bias']
        )  # 0.9999987105141137
        output_classes_shape = outputs_classes.shape # type: ignore[attr-defined]
        # agent_conf = outputs_classes[-1]
        agent_conf = ttnn.Tensor(shape=(output_classes_shape[1], output_classes_shape[2], output_classes_shape[3]), dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        ...
        for i in range(4):
            if i in [0, 3]:  # Linear layers
                layer_params = agent_fus_mlp_params[str(i)]
                cls_tmp = ttnn.linear(
                    cls_tmp,
                    layer_params["weight"],
                ...
                norm_params = agent_fus_mlp_params["1_norm"]
                cls_tmp = ttnn.layer_norm(
                    cls_tmp,
        ...
        map_hs_shape = map_hs.shape
        # map_query = map_hs[-1]
        map_query = ttnn.Tensor(shape=(map_hs_shape[1], map_hs_shape[2], map_hs_shape[3]), dtype=ttnn.bfloat16, device=self.device)
        ...
        map_outputs_classes_shape = map_outputs_classes.shape # type: ignore[attr-defined]
        map_outputs_coords_bev_shape = map_outputs_coords_bev[-1].shape
        ...
        x = ttnn.Tensor(shape=(map_pos_shape[0], map_pos_shape[1], map_pos_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        y = ttnn.Tensor(shape=(map_pos_shape[0], map_pos_shape[1], map_pos_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        const_2 = ttnn.full(shape=x.shape, fill_value=2.0, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.ROW_MAJOR_LAYOUT)
        x_sq = ttnn.pow(x, const_2)
        ...
        min_map_pos_idx = ttnn.argmax((map_dis * -1), axis=-1)  # [B, P]
        ...
        min_map_pos = ttnn.Tensor(shape=(min_map_pos.shape[0], 2), dtype=ttnn.bfloat16, device=self.device)
        ...
        B, _, NP = map_outputs_coords.shape # type: ignore[attr-defined]
        map_outputs_coords = ttnn.reshape(map_outputs_coords, (B, 1, NP//4, 4))
        outs = {
            "bev_embed": bev_embed,
            "all_cls_scores": outputs_classes,
            "all_bbox_preds": outputs_coords,
            "all_traj_preds": ttnn.repeat(outputs_trajs, (outputs_coords.shape[0], 1, 1, 1, 1)), # type: ignore[attr-defined]
            "all_traj_cls_scores": ttnn.repeat(outputs_trajs_classes, (outputs_coords.shape[0], 1, 1, 1)), # type: ignore[attr-defined]
    ...
    def map_transform_box(self, pts, y_first=False):
        ...
        pts_01 = ttnn.Tensor(shape=(pts_reshape.shape[0], pts_reshape.shape[1], pts_reshape.shape[2]), dtype=ttnn.bfloat16, device=self.device)
        pts_x = pts_01
        pts_y = pts_01
        if self.map_transform_method == "minmax":
            ...
            bbox = ttnn.concat(xmin, ymin, xmax, ymax, axis=-1, memory_config=ttnn.L1_MEMORY_CONFIG)
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            bboxes = box_type_3d(bboxes, code_size) # type: ignore[abstract]
            scores = preds["scores"]
            labels = preds["labels"]
            trajs = preds["trajs"]
    ...
    def select_and_pad_pred_map(
        ...
        x = ttnn.Tensor(shape=(map_pos_shape[0], map_pos_shape[1], map_pos_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        y = ttnn.Tensor(shape=(map_pos_shape[0], map_pos_shape[1], map_pos_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        const_2 = ttnn.full(shape=x.shape, fill_value=2.0, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        x_sq = ttnn.pow(x, const_2)
        y_sq = ttnn.pow(y, const_2)
        result = ttnn.add(x_sq, y_sq)
        map_dis = ttnn.sqrt(result)
        map_dis = ttnn.to_layout(map_dis, ttnn.ROW_MAJOR_LAYOUT)
        map_dis_m1 = map_dis * -1
        min_map_pos_idx = ttnn.argmax(map_dis_m1, dim=-1)  # [B, P]
        min_map_pos_idx = ttnn.reshape(min_map_pos_idx, [-1])
        min_map_pos = ttnn.reshape(map_pos, [map_pos.shape[0] * map_pos.shape[1], map_pos.shape[2], map_pos.shape[3]])
        min_map_pos = ttnn.to_torch(min_map_pos)
        min_map_pos_idx = ttnn.to_torch(min_map_pos_idx)

        min_map_pos_shape = min_map_pos.shape
        min_map_pos = ttnn.Tensor(shape=(min_map_pos_shape[0], min_map_pos_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        ...
        map_score = ttnn.sigmoid(map_score) # replace sigmoid_accurate with sigmoid
        map_max_score = ttnn.max(map_score, dim=-1) #[0]
        map_thresh_tensor = ttnn.full(shape=map_max_score.shape, fill_value=map_thresh, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        map_idx = ttnn.compare(map_max_score, map_thresh_tensor, "greater")  # [B, P]

        selected_map_query = ttnn.Tensor(shape=[1,3,256], dtype=ttnn.bfloat16, device=self.device)
        selected_map_pos = ttnn.Tensor(shape=[1,3,2], dtype=ttnn.bfloat16, device=self.device)
        selected_padding_mask = ttnn.Tensor(shape=[1,3], dtype=ttnn.bfloat16, device=self.device)
        ...
        selected_map_dist_shape = selected_map_dist.shape
        x = ttnn.Tensor(shape=(selected_map_dist_shape[0], selected_map_dist_shape[1], selected_map_dist_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        y = ttnn.Tensor(shape=(selected_map_dist_shape[0], selected_map_dist_shape[1], selected_map_dist_shape[2]), dtype=ttnn.bfloat16, device=self.device)
        const_2 = ttnn.full(shape=x.shape, fill_value=2.0, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        x_sq = ttnn.pow(x, const_2)
        y_sq = ttnn.pow(y, const_2)
        result = ttnn.add(x_sq, y_sq)
        map_dis = ttnn.sqrt(result)
        dis_thresh_tensor = ttnn.full(shape=map_dis.shape, fill_value=dis_thresh, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        valid_map_inst = ttnn.compare(map_dis, dis_thresh_tensor, "less_equal") # [B, A, max_P]
        ...
        if use_fix_pad:
            batch_size = int(selected_map_pos.shape[0])
            feat_dim_int = int(selected_map_query.shape[-1])
            pad_map_query = ttnn.full(shape=(batch_size, 1, feat_dim_int), fill_value=0.0, device=self.device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
            pad_map_pos = ttnn.full(shape=(batch_size, 1, 2), fill_value=1.0, device=self.device, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT)
            pad_lane_mask = ttnn.full(
                 shape=(num_batch, 1),
                 fill_value=False,
                 dtype=ttnn.bool,
                 device=self.device,
            ...
            if TYPE_CHECKING:
                assert selected_map_pos.shape is not None
                assert pad_map_pos.shape is not None
    ...
    def select_and_pad_query(self, query, query_pos, query_score, score_thresh=0.5, use_fix_pad=True):
        ...
        score_thresh_tensor = ttnn.full(shape=query_score.shape, fill_value=score_thresh, dtype=ttnn.bfloat16, device=self.device, layout=ttnn.TILE_LAYOUT)
        query_idx = ttnn.compare(query_score, score_thresh_tensor, "greater")
        ...
        selected_padding_mask = ttnn.Tensor(shape=[1,100], dtype=ttnn.bfloat16, device=self.device)
