...
class RetinaNet(SimNN.Module):
    ...
    def create_input_tensors(self) -> None:
        ...
        - x_in: dummy image input [1, 3, img_size, img_size]
        ...
        anchors_t = F._from_data(f"{self.name}_anchors", anchors_np)
    ...
    def __call__(self, x=None):
        # Ensure anchors_in (and x_in) are created exactly once
        if not hasattr(self, "input_tensors"):
            self.create_input_tensors()

        # If caller didn't pass x, use the internally created x_in
        if x is None:
    ...
    def get_forward_graph(self):
        # Ensure inputs and graph are built
        if not hasattr(self, "input_tensors"):
            self.create_input_tensors()

        # Run once to materialize all ops/tensors
        _ = self()
